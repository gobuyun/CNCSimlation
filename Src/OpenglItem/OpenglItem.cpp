...
int RenderProcess::initRenderContent(RenderContent** renderContent, const QSize &size)
{
    try
    {
        ...
        QString toolPath = "100F/tool.stl";
        QVector<QVector<int>> axisHierarchy = {{},{0},{}};
...
void OpenglItem::handCmd(unsigned int jog_id, int jog_dir)
{
    MachineControl::handCmd(jog_id, jog_dir);
}

void OpenglItem::positionCmd(std::vector<double> axisPe)
{
    MachineControl::positionCmd(m_curAxisVal, axisPe);
}

void OpenglItem::autoCmd(QStringList axisvAalStr)
{
    std::vector<std::vector<double>> axisvAal;
    foreach(auto line, axisvAalStr)
    {
        auto aixsParts = line.split(" ");
        std::vector<double> axisLineVal;
        if (aixsParts.size() >= 3) // 值判断
        {
            for (auto it: aixsParts)
            {
                axisLineVal.push_back(it.mid(1, it.size()-1).toDouble());
            }
        }
        axisvAal.push_back(axisLineVal);
    }
//    MachineControl::execAutoCmd(axisvAal[0]);
    MachineControl::autoCmd(axisvAal);
}
bool OpenglItem::autoRunCanPush()
{
    return MachineControl::autoCmdCacheIsEmpty();
}
void OpenglItem::stopAutoCmd()
{
    return MachineControl::stopAutoCmd();
}

void OpenglItem::updateCurAxisVal()
{
    std::vector<double> axisvAal;
    MachineControl::getAxisCurVal(axisvAal);
    m_curAxisVal = axisvAal;
    emit curAxisValChanged();
}
