...
// = Rotations
	...
	static Rotation GetVectorRotate(Vector v1, Vector v2);
...
/**     Gives back a rotation matrix specified with EulerZYZ convention :
    ...
    static Rotation EulerZXZ(double Alfa,double Beta,double Gamma);
...
class Twist {
	...
	/*
	A Twist is the 6x1 matrix that represents the 
	velocity of a Frame using a 3D translational 
	velocity Vector vel and a 3D angular velocity Vector rot:
			[vx]
			[vy]
			[vz]
	Twist = [rx]
...
{
	/* 
	A Wrench is the 6x1 matrix that represents
	a force on a Frame using a 3D translational 
	force Vector force and a 3D moment Vector torque:
		   [fx]
		   [fy]
		   [fz]
	Wrench=[tx]
