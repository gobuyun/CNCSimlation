...
Rotation Rotation::EulerZXZ(double Alfa, double Beta, double Gamma)
{
        double sa,ca,sb,cb,sg,cg;
        sa  = sin(Alfa);ca = cos(Alfa);
        sb  = sin(Beta);cb = cos(Beta);
        sg  = sin(Gamma);cg = cos(Gamma);
        return Rotation( -sa*cb*sg+ca*cg,     -sa*cb*cg-ca*cg,        sa*sb,
...
double Rotation::GetRotAngle(Vector& axis,double eps) const {
    ...
    double epsilon_ = eps; // margin to allow for rounding errors
    double epsilon2 = eps*10; // margin to distinguish between 0 and 180 degrees

    // optional check that input is pure rotation, 'isRotationMatrix' is defined at:
    // http://www.euclideanspace.com/maths/algebra/matrix/orthogonal/rotation/

    if ((std::abs(data[1] - data[3]) < epsilon_)
        && (std::abs(data[2] - data[6])< epsilon_)
        && (std::abs(data[5] - data[7]) < epsilon_))
    {
        // singularity found
...
/* 
 Returns the rotation matrix from v1 to v2
*/
Rotation Rotation::GetVectorRotate(Vector v1, Vector v2)
{
	v1.Normalize();
	v2.Normalize();
	Vector normalV =v1*v2;
	double angel = acos(dot(v1, v2));
	return Rotation::Rot(normalV, angel);
}
